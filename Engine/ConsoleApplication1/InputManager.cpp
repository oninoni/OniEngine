...
void InputManager::scroll_callback(GLFWwindow * window, double xoffset, double yoffset) {
    if (yoffset != 0) {
        cout << yoffset << endl;
        if (yoffset > 0) {
            getInstance()->getScrollData()[IM_SCROLL_UP] = (int)yoffset;
        }
        else {
            getInstance()->getScrollData()[IM_SCROLL_DOWN] = (int)yoffset;
        }
    }
}
...
InputManager* InputManager::getInstance() {
    ...
    static InputManager input(MainComponent::getInstance()->getGLFWWindow());
    return &input;
}
